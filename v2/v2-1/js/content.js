...
let newGridInfo = {
  ...
  idIndex: (index) =>
    `<div id="infoIndex${index}" class="infoStyle">${index}</div>`,
  idExpCon: (index) =>
    `<div id="infoExpCon${index}" class="infoStyle" onClick="expand(${index});">+</div>`,
  idTime: (index, time) =>
    `<div id="infoTime${index}" class="infoStyle">${time}</div>`,
  idStartAdd: (index, startAdd) =>
    `<div id="infoStartAdd${index}" class="infoStyle">${startAdd}</div>`,
  idRunTotal: (runTotal) =>
    `<div id="infoRunTotal" class="infoStyle">${runTotal}</div>`,
  ...
  idDetailsEndTime: (index, detailsArrival) =>
    `<div id= detailsEndTime${index}" class="detailsStyle">Arrival Time: ${detailsArrival}</div>`,
  idDetailsDestination: (index, detailsDestination) =>
    `<div id="detailsDestination${index}" class="detailsStyle">Destination Address: ${detailsDestination}</div>`,
  idDetailsFareTotal: (index, detailsFareTotal) =>
    `<div id="detailsFareTotal${index}" class="detailsStyle">Fare Total: ${detailsFareTotal}</div>`,
  clSpaceB1: `<div class="space"></div>`,
  clSpaceB2: `<div class="space"></div>`,
  clSpaceB3: `<div class="space"></div>`,
  clSpaceB4: `<div class="space"></div>`,
  idDetailsTotalTime: (index, totalTime) =>
    `<div id="detailsTotalTime${index}" class="detailsStyle">Total Time: ${totalTime}</div>`,
  idDetailsDistance: (index, detailsDistance) =>
    `<div id="detailsDistance${index}" class="detailsStyle">Distance (km): ${detailsDistance}</div>`,
  idDetailDollarsPerKm: (index, detailsDolPerKm, detailsDistance) =>
    `<div id="detailsDollarsPerKm${index}" class="detailsStyle">$/km: ${dollarsPerKm(
      detailsDolPerKm,
      detailsDistance
    )}</div>`,
  ...
  idDetailsFareType: (index, detailsFareType) =>
    `<div id="detailsFareType${index}" class="detailsStyle">FareType: ${detailsFareType}</div>`,
  idDetailsTip: (index, detailsTip) =>
    `<div id="detailsTip${index}" class="detailsStyle">Tip: ${detailsTip}</div>`,
...
let newTotals = {
  ...
  idTotalsShiftMeter: (shiftMeter) =>
    `<div id="totalsShiftMeter" class="totalsContent">Meter: ${shiftMeter}</div>`,
  idTotalsShiftFlat: (shiftFlat) =>
    `<div id="totalsShiftFlat" class="totalsContent">Flat Rates: ${shiftFlat}</div>`,
  idTotalsShiftTips: (shiftTips) =>
    `<div id="totalsShiftTips" class="totalsContent">Tips: ${shiftTips}</div>`,
  idTotalsShift: (shiftTotal) =>
    `<div id="totalsShift" class="totalsFooter">Total Shift: ${shiftTotal}</div>`,
  clTotalsProfHeader: `<div class="totalsHeader">Profit Report</div>`,
  idTotalsProfitBroker: (profitBroker) =>
    `<div id="totalProfitBroker" class="totalsContent">Broker Profit: ${profitBroker}</div>`,
  idTotalsProfitDriver: (profitDriver) =>
    `<div id="totalsProfitDriver" class="totalsContent">Driver Profit ${profitDriver}</div>`,
  idTotalsProfit: (profitTotal) =>
    `<div id="totalsProfit" class="totalsFooter">Total Taxi Profit: ${profitTotal}</div>`,
  clTotalsPayHeader: `<div class="totalsHeader">Payment Type Report</div>`,
  idTotalsPayCash: (payCash) =>
    `<div id="totalsPayCash" class="totalsContent">Cash: ${payCash}</div>`,
  idTotalsPayDebit: (payDebit) =>
    `<div id="totalsPayDebit" class="totalsContent">Debit/Credit: ${payDebit}</div>`,
  idTotalsPayOther: (payOther) =>
    `<div id="totalsPayOther" class="totalsContent">Other: ${payOther}</div>`,
  idTotalsPay: (payTotal) =>
    `<div id="totalsPay" class="totalsFooter">Total Gross Paid: ${payTotal}</div>`,
  clTotalsKmHeader: `<div class="totalsHeader">KM Report</div>`,
  idTotalsKmTraveled: (kmTraveled) =>
    `<div id="totalsKmTraveled" class="totalsContent">Km Traveled: ${kmTraveled}</div>`,
  idTotalsKmMetered: (kmMetered) =>
    `<div id="totalsKmMetered" class="totalsContent">Metered Km: ${kmMetered}</div>`,
  idTotalsKmDolPerKm: (kmPer) =>
    `<div id="totalsKmDolPerKm" class="totalsContent">$/km: ${kmPer}</div>`,
  clTotalsHeader: `<div class="totalsHeader">Expense Report</div>`,
  idTotalsExpPersonal: (expPersonal) =>
    `<div id="totalsExpPersonal" class="totalsContent">Personal Expenses: ${expPersonal}</div>`,
  idTotalsExpTaxi: (expTaxi) =>
    `<div id="totalsExpTaxi" class="totalsContent">Taxi Expenses: ${expTaxi}</div>`,
  idTotalsExp: (expTotal) =>
    `<div id="totalsExp" class="totalsFooter">Total Expenses: ${expTotal}</div>`,
  idTotalsEstGas: (estGas) =>
    `<div id="totalsEstGas" class="totalsHeader">Estimated Gas Total: ${estGas}</div>`,
  closeDiv: `</div>`,
};
...
addNewFareTable = (dropTime, formattedDropTime) => {
  ...
  const addFare = elementFromHtml(
    ...
    newGridInfo.idGridIndfo +
      ...
      newGridInfo.idDetailDollarsPerKm(
        fareTableCount,
        detailsFareTotal,
        detailsDistance
      ) +
